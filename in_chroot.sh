...
emerge -uvDN sys-kernel/gentoo-sources:5.14.21 sys-kernel/genkernel
if [[ $? -eq 0 ]]; then
    echo "SUCCESS: 'emerge -uvDN sys-kernel/gentoo-sources-5.14.21 sys-kernel/genkernel' done."
else
    echo "Error: 'emerge -uvDN sys-kernel/gentoo-sources-5.14.21 sys-kernel/genkernel' failed."
    exit 1
fi
eselect kernel list
